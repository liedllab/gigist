...
float dist2_imageNonOrthoRecipTest(float * vec1, float * vec2, UnitCell ucell) {
  ...
  y -= rintf(y);
  z -= rintf(z);
  float fx = x * ucell[0] + y * ucell[3] + z * ucell[6];
  float fy = x * ucell[1] + y * ucell[4] + z * ucell[7];
  float fz = x * ucell[2] + y * ucell[5] + z * ucell[8];
  float ret = fx * fx + fy * fy + fz * fz;
  return ret;
}
...
void calculateEntropy(EntropyCalculator entCalc, float *returnSTrans, float *returnSOrient, float *returnSSix) {
  ...
  entCalc.calcTransEntropy(index);
  entCalc.calcOrientEntropy(index);
  returnSOrient[index] = entCalc.dTSorient;
  returnSTrans[index] = entCalc.dTStrans;
  returnSSix[index] = entCalc.dTSsix;
}